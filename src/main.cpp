...
int main() {
  ...
  // macOS requires CAP_AVFOUNDATION; Windows/Linux use default backend
#ifdef __APPLE__
  ...
  // Create gesture classifier and action mapper
  GestureClassifier classifier;
  ActionMapper mapper(2.0); // 2-second cooldown between actions
  std::string lastAction;   // Last triggered action for display
      ...
      for (const auto &[start, end] : connections) {
        if (start < hand.landmarks.size() && end < hand.landmarks.size()) {
          ...
          if (p1.x > 0 && p1.y > 0 && p2.x > 0 && p2.y > 0 &&
              p1.x < frame.cols && p1.y < frame.rows && p2.x < frame.cols &&
              p2.y < frame.rows) {
