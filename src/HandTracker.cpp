...
bool HandTracker::startPythonProcess() {
    ...
    std::string python_path = "/Users/selo/group31/.venv/bin/python3";
    std::string cmd = python_path + " /Users/selo/group31/hand_detector.py 2>&1";
...
std::vector<Hand> HandTracker::detectHands(const cv::Mat& frame) {
    ...
    if (!initialized_ || !python_process_) return hands;
    ...
    try {
        ...
        if (fgets(buffer, sizeof(buffer), python_process_) != nullptr) {
            ...
            if (frame_count == 1 || frame_count % 60 == 0) {
                ...
                std::string preview = json_str.substr(0, std::min(size_t(300), json_str.length()));
            ...
            std::string hands_content = json_str.substr(array_open + 1, array_close - array_open - 1);
            ...
            while (pos < hands_content.length()) {
                ...
                while (pos < hands_content.length() && (hands_content[pos] == ' ' || hands_content[pos] == '\t' || 
                       hands_content[pos] == '\n' || hands_content[pos] == ',')) {
                    pos++;
                }
                ...
                std::string hand_obj_str = hands_content.substr(hand_start, hand_end - hand_start + 1);
                pos = hand_end + 1;
                parsed_hands++;
                ...
                if (hnd_pos != std::string::npos) {
                    ...
                    if (quote1 != std::string::npos && quote2 != std::string::npos) {
                        hand.handedness = hand_obj_str.substr(quote1 + 1, quote2 - quote1 - 1);
                    }
                }
                ...
                if (conf_pos != std::string::npos) {
                    ...
                    std::string conf_str = hand_obj_str.substr(val_start, val_end - val_start);
                    try {
                ...
                if (lm_key != std::string::npos) {
                    ...
                    if (lm_array_open != std::string::npos) {
                        ...
                        while (lm_array_close < hand_obj_str.length()) {
                            if (hand_obj_str[lm_array_close] == '[') lm_bracket++;
                            else if (hand_obj_str[lm_array_close] == ']') {
                                lm_bracket--;
                                if (lm_bracket == 0) break;
                            }
                            lm_array_close++;
                        }
                        
                        if (lm_bracket == 0) {
                            std::string lm_content = hand_obj_str.substr(lm_array_open + 1, lm_array_close - lm_array_open - 1);
                            
                            // Parse each landmark
                            size_t lm_pos = 0;
                            while (lm_pos < lm_content.length()) {
                                // Skip whitespace
                                while (lm_pos < lm_content.length() && (lm_content[lm_pos] == ' ' || lm_content[lm_pos] == '\t' || 
                                       lm_content[lm_pos] == '\n' || lm_content[lm_pos] == ',')) {
                                    lm_pos++;
                                }
                                
                                if (lm_pos >= lm_content.length() || lm_content[lm_pos] != '{') break;
                                ...
                                std::string lm_obj = lm_content.substr(lm_start, lm_end - lm_start + 1);
                                lm_pos = lm_end + 1;
            ...
            if (frame_count == 1 || frame_count % 60 == 0) {
                std::cerr << "Frame " << frame_count << ": Found " << hands.size() << " hands\n";
            }
        }
